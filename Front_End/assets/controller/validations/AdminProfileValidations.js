...
adminValidations.push({
    reg: regCusID,
    field: $('#user_Id'),
    error: 'Admin ID Pattern is Wrong'
});
adminValidations.push({
    reg: regFirstName,
    field: $('#firstName'),
    error: 'Admin First Name Pattern is Wrong'
});
adminValidations.push({
    reg: regLastName,
    field: $('#lastName'),
    error: 'Admin Last Name Pattern is Wrong'
});
adminValidations.push({
    reg: regContactNum,
    field: $('#contact_No'),
    error: 'Admin Contact Number Pattern is Wrong'
});
adminValidations.push({
    reg: regCusAddress,
    field: $('#address'),
    error: 'Admin Address Pattern is Wrong'
});
adminValidations.push({
    reg: regEmailCusAddress,
    field: $('#email'),
    error: 'Admin Email Address Pattern is Wrong'
});
adminValidations.push({
    reg: regNIC,
    field: $('#nic'),
    error: 'Admin NIC Pattern is Wrong'
});
adminValidations.push({
    reg: regUserName,
    field: $('#user_Name'),
    error: 'Admin User Name Pattern is Wrong'
});
adminValidations.push({
    reg: regPassword,
    field: $('#password'),
    error: 'Admin Password Pattern is Wrong'
});
...
function setButtonState(value) {
    if (value > 0) {
        $("#btnUpdateAdminForm").attr('disabled', true);
    } else {
        $("#btnUpdateAdminForm").attr('disabled', false);
    }
}
